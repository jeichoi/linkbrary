...
// ---- Link ----
let deleteLinkCallback: ((id: number) => void) | null = null;

export function setDeleteLinkCallback(cb: (id: number) => void) {
  deleteLinkCallback = cb;
}

export function runDeleteLinkCallback(id: number) {
  if (deleteLinkCallback) {
    deleteLinkCallback(id);
    deleteLinkCallback = null;
  }
}

// ---- Link ----
let editLinkCallback: ((updatedLink: Link) => void) | null = null;

export function setEditLinkCallback(cb: (updatedLink: Link) => void) {
  editLinkCallback = cb;
}

export function runEditLinkCallback(updatedLink: Link) {
  if (editLinkCallback) {
    editLinkCallback(updatedLink);
    editLinkCallback = null;
  }
}

// ---- Folder ----
let deleteFolderCallback: ((id: number) => void) | null = null;

export function setDeleteFolderCallback(cb: (id: number) => void) {
  deleteFolderCallback = cb;
}

export function runDeleteFolderCallback(id: number) {
  if (deleteFolderCallback) {
    deleteFolderCallback(id);
    deleteFolderCallback = null;
  }
}

// ---- Folder ----
let editFolderCallback: ((updatedFolder: Folder) => void) | null = null;

export function setEditFolderCallback(cb: (updatedFolder: Folder) => void) {
  editFolderCallback = cb;
}

export function runEditFolderCallback(updatedFolder: Folder) {
  if (editFolderCallback) {
    editFolderCallback(updatedFolder);
